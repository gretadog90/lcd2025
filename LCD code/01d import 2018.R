################################################################
# Import 2018 GEE data to create the yearly NDVI metrics
################################################################
...
setwd('~/Documents/data/Lancet 2025/2018')
...
#for each file in file list, rename to drop "2018_" and ".csv" and read in csv as object
for (i in 1:length(file_list)) assign(str_remove(str_remove(file_list[i], "2018_"),".csv"), 
...
seasons<- seasons %>%
  ...
  summarize(Avg_NDVI_2018 = mean(mean, na.rm = TRUE), Peak_NDVI_2018=max(mean, na.rm = TRUE))
...
seasons_popw_100m<- seasons_popw_100m %>%
  group_by(ID_HDC_G0, city, country, UC_NM_MN, XC_ISO_LST) %>%
  summarize(annual_avg_num_2018_100m = mean(sum, na.rm = TRUE), 
            annual_max_num_2018_100m = max(sum, na.rm = TRUE) )
...
seasons_popw_100m$PopWeight_Avg_NDVI_2018_100m<-seasons_popw_100m$annual_avg_num_2018_100m/seasons_popw_100m$Population_2015_100m
seasons_popw_100m$PopWeight_Peak_NDVI_2018_100m<-seasons_popw_100m$annual_max_num_2018_100m/seasons_popw_100m$Population_2015_100m

#get rid of a couple of unnecessary vars
seasons_popw_100m[c("annual_avg_num_2018_100m", 
                    "annual_max_num_2018_100m")] <- list(NULL) 

data2018<-seasons %>%
  inner_join(seasons_popw_100m)

write.csv(data2018, paste0(output_dir, 'data2018.csv'))