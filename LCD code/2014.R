################################################################
# Import 2014 GEE data to create the yearly NDVI metrics
################################################################
...
setwd('~/Documents/data/Lancet 2025/2014')
...
#for each file in file list, rename to drop "2014_" and ".csv" and read in csv as object
for (i in 1:length(file_list)) assign(str_remove(str_remove(file_list[i], "2014_"),".csv"), 
...
seasons_popw_100m<- seasons_popw_100m %>%
  ...
  summarize(annual_avg_num_2014_100m = mean(sum, na.rm = TRUE), 
            annual_max_num_2014_100m = max(sum, na.rm = TRUE) )
...
seasons_popw_100m$PopWeight_Avg_NDVI_2014_100m<-seasons_popw_100m$annual_avg_num_2014_100m/seasons_popw_100m$Population_2015_100m
seasons_popw_100m$PopWeight_Peak_NDVI_2014_100m<-seasons_popw_100m$annual_max_num_2014_100m/seasons_popw_100m$Population_2015_100m

#get rid of a couple of unnecessary vars
seasons_popw_100m[c("annual_avg_num_2014", "annual_max_num_2014", "annual_avg_num_2014_100m", 
                    "annual_max_num_2014_100m")] <- list(NULL) 

write.csv(seasons_popw_100m, paste0(output_dir, 'data2014.csv'))
