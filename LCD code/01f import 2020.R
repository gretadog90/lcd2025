################################################################
# Import 2020 GEE data to create the yearly NDVI metrics
################################################################
...
setwd('~/Documents/data/Lancet 2025/2020')
...
for (i in 1:length(file_list)) assign(str_remove(str_remove(file_list[i], "2020_"),".csv"), 
...
seasons<- seasons %>%
  ...
  summarize(Avg_NDVI_2020 = mean(mean, na.rm = TRUE), Peak_NDVI_2020=max(mean, na.rm = TRUE))
...
seasons_popw<- seasons_popw %>%
  group_by(ID_HDC_G0, city, country, UC_NM_MN, XC_ISO_LST) %>%
  summarize(annual_avg_num_2020 = mean(sum, na.rm = TRUE), annual_max_num_2020 = max(sum, na.rm = TRUE) )
...
seasons_popw$PopWeight_Avg_NDVI_2020<-seasons_popw$annual_avg_num_2020/seasons_popw$Population_2020
seasons_popw$PopWeight_Peak_NDVI_2020<-seasons_popw$annual_max_num_2020/seasons_popw$Population_2020
...
seasons_popw_100m<- seasons_popw_100m %>%
  group_by(ID_HDC_G0, city, country, UC_NM_MN, XC_ISO_LST) %>%
  summarize(annual_avg_num_2020_100m = mean(sum, na.rm = TRUE), 
            annual_max_num_2020_100m = max(sum, na.rm = TRUE) )
...
seasons_popw_100m$PopWeight_Avg_NDVI_2020_100m<-seasons_popw_100m$annual_avg_num_2020_100m/seasons_popw_100m$Population_2020_100m
seasons_popw_100m$PopWeight_Peak_NDVI_2020_100m<-seasons_popw_100m$annual_max_num_2020_100m/seasons_popw_100m$Population_2020_100m

#### Green and blue area ####
#create new data frame to hold ga and gba
landcover <- data.frame(ID_HDC_G0 = ga$ID_HDC_G0, Green_Area_2020 = ga$mean,
                        GreenBlue_Area_2020=gba$mean
)

#### Combine all 2020 measures together based measures ####
data2020<-seasons %>%
...
data2020[c("annual_avg_num_2020", "annual_max_num_2020", "annual_avg_num_2020_100m", 
           "annual_max_num_2020_100m")] <- list(NULL) 

write.csv(data2020, paste0(output_dir, 'data2020.csv'))
