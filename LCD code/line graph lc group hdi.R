...
means_who_sub <- aggregate(long$value, by=list(long$lc_group, long$hdi_level, long$year), 
                           function(x)mean(x, na.rm=TRUE))
# rename columns to match
colnames(means_who_sub) <- c("lc_group","hdi_level","year", "mean")
...
#plot
pdf(file = "LCD indicator report/line graph averages lc group hdi.pdf", width=13.5, height=8.5)
a<-ggplot() +
  geom_line(data=long, aes(x=year, y=value, group=city, color=hdi_level), size=.3) +
  geom_line(data=means_who_sub, aes(x=year, y=mean,group=lc_group, color=hdi_level), size=1)+
  scale_color_manual(values=c("#E66101","#FDB863","#DEE6E7","#B2ABD2", "#5E3C99"), name = "Human Development Index")+
  xlab('Year') + ylab('Population-weighted greenest season NDVI')+ facet_wrap(~lc_group, ncol=4)+ylim(0, 0.6)+
