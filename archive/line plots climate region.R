...
subset<-subset(subset, clim_region!="Polar")
...
means_who_sub <- aggregate(long$value, by=list(long$clim_region, long$year), 
                           function(x)mean(x, na.rm=TRUE))
# rename columns to match
colnames(means_who_sub) <- c("clim_region","year", "mean")
...
#plot
pdf(file = "graphs/line graph averages clim region.pdf", width=13.5, height=8.5)
ggplot() +
  geom_line(data=long, aes(x=year, y=value, group=city, color=clim_region), alpha=.2, size=.5) +
  scale_color_brewer(palette = "PuOr")+
  geom_line(data=means_who_sub, aes(x=year, y=mean, group=clim_region), size=1) +
  xlab('Year') + ylab('Population-weighted greenest season NDVI')+ facet_wrap(~clim_region)
dev.off()
