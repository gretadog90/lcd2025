################################################################
# Import 2023 GEE data to create the yearly NDVI metrics
################################################################
...
setwd('~/Documents/data/Lancet 2025/2023')
...
#for each file in file list, rename to drop "2023_" and ".csv" and read in csv as object
for (i in 1:length(file_list)) assign(str_remove(str_remove(file_list[i], "2023_"),".csv"), 
...
seasons<- seasons %>%
  ...
  summarize(Avg_NDVI_2023 = mean(mean, na.rm = TRUE), Peak_NDVI_2023=max(mean, na.rm = TRUE))
...
seasons_popw<- seasons_popw %>%
  group_by(ID_HDC_G0, city, country, UC_NM_MN, XC_ISO_LST) %>%
  summarize(annual_avg_num_2023 = mean(sum, na.rm = TRUE), annual_max_num_2023 = max(sum, na.rm = TRUE) )
...
seasons_popw$PopWeight_Avg_NDVI_2023<-seasons_popw$annual_avg_num_2023/seasons_popw$Population_2020
seasons_popw$PopWeight_Peak_NDVI_2023<-seasons_popw$annual_max_num_2023/seasons_popw$Population_2020
...
seasons_popw_100m<- seasons_popw_100m %>%
  group_by(ID_HDC_G0, city, country, UC_NM_MN, XC_ISO_LST) %>%
  summarize(annual_avg_num_2023_100m = mean(sum, na.rm = TRUE), 
            annual_max_num_2023_100m = max(sum, na.rm = TRUE) )
...
seasons_popw_100m$PopWeight_Avg_NDVI_2023_100m<-seasons_popw_100m$annual_avg_num_2023_100m/seasons_popw_100m$Population_2020_100m
seasons_popw_100m$PopWeight_Peak_NDVI_2023_100m<-seasons_popw_100m$annual_max_num_2023_100m/seasons_popw_100m$Population_2020_100m

#### Combine all 2023 measures together based measures ####
data2023<-seasons %>%
  inner_join(seasons_popw) %>% 
  inner_join(seasons_popw_100m)

#get rid of a couple of unnecessary vars
data2023[c("annual_avg_num_2023", "annual_max_num_2023", "annual_avg_num_2023_100m", 
           "annual_max_num_2023_100m")] <- list(NULL) 

write.csv(data2023, paste0(output_dir, 'data2023.csv'))
